...
- 展示環境での表示安定化
- Framing Preview Indicator によるスマホ側限定の構図判定表示
...
制作期間中は、メンバーごとの作業状況を確認し、必要に応じてタスクの切り分けや代替案の検討を行いました。作業量が偏った場合は、担当範囲を調整したり、共同作業日を設けたりすることで、限られた制作期間内で完成度を上げることを優先しました。
...
| Framing Preview Indicator | 試遊でスマホファインダーを見ないプレイヤーが多かった課題に対し、撮影前に「全身OK / 上半身OK / 下半身OK / 構図を調整」をスマホ側だけへ表示する機能を追加しました。既存の撮影スコア判定を `EvaluatePhotoFraming()` として副作用なしに分離し、撮影時とプレビュー時で同じ判定を使うことで、UI表示と実スコアのズレを防いでいます。 |

## Framing Preview Indicator

試遊では、スマホファインダー自体には新規性がある一方で、ズーム中もメインモニターを見続けるプレイヤーが多い課題がありました。スマホ画面を見なくても最低限プレイできるため、スマホを見る必然性がまだ弱い状態でした。

そこで、撮影スコアに関係する構図判定を、スマホ側にだけリアルタイム表示する **Framing Preview Indicator** を追加しました。

表示内容は以下です。

| 表示 | 意味 |
|------|------|
| 全身OK | 全身が映っており、スコア100% |
| 上半身OK | 上半身のみ映っており、スコア50% |
| 下半身OK | 下半身のみ映っており、スコア50% |
| 構図を調整 | 撮影評価が低い、または対象が十分に映っていない |

この機能は、メイン画面ではなくスマホ側の `PhoneViewWidget` にのみ表示しています。メイン画面にも同じ情報を出すと、スマホを見る理由が弱くなるためです。

実装では、既存の撮影スコア判定をスマホ用に作り直すのではなく、`UTomatinaFunctionLibrary::CalculatePhotoScore()` にあった Head / Root の可視判定部分を `EvaluatePhotoFraming()` として分離しました。`EvaluatePhotoFraming()` は判定結果を返すだけで、スコア加算、ミッション進行、SE再生、リザルト表示、ターゲット削除などの副作用を持ちません。

撮影時は `ATomatinaGameMode::TakePhoto()` が同じ判定結果を使ってスコアへ反映し、プレビュー時は `ATomatinaHUD` がその結果を `PhoneViewWidget` へ渡して表示します。撮影時とプレビュー時で同じ判定ロジックを使うことで、UI表示と実際のスコア判定がズレないようにしています。

### 変更ファイル

- `Source/Tomato/TomatinaFunctionLibrary.h`
- `Source/Tomato/TomatinaFunctionLibrary.cpp`
- `Source/Tomato/TomatinaGameMode.h`
- `Source/Tomato/TomatinaGameMode.cpp`
- `Source/Tomato/TomatinaHUD.h`
- `Source/Tomato/TomatinaHUD.cpp`
- `Source/Tomato/TomatinaPlayerPawn.h`
- `Source/Tomato/TomatinaPlayerPawn.cpp`

### 今後の改善

現在はTextBlock中心の最低限の表示なので、今後は色、アイコン、被写体枠、撮影直前の強調演出を追加し、構図の良し悪しがより直感的に伝わるように改善したいです。
...
| # | 課題 | 重要度 | 現状・原因 | 改善方針 |
|---|------|--------|------------|----------|
| 1 | スマホファインダーへの視線誘導のさらなる改善 | 中 | Framing Preview Indicator により、撮影スコアに関係する構図判定をスマホ側だけへ表示するところまでは実装済みです。ただし、現状はTextBlock中心の表示で、視覚的な分かりやすさには改善余地があります。 | 色、アイコン、被写体枠、撮影直前の強調演出を追加し、構図の良し悪しがより直感的に伝わるようにする。 |
